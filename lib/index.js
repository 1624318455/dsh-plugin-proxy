...
export function buildSystemDispatcher(detected) {
	...
	if (detected?.httpProxy || detected?.httpsProxy) {
		...
		const proxied = new EnvHttpProxyAgent({ httpProxy: http, httpsProxy: https, noProxy: '' });
		return rules.length ? new RoutingDispatcher(new Agent(), proxied, rules) : proxied;
	}
	if (detected?.socksProxy) {
		const proxied = new Socks5ProxyAgent(detected.socksProxy);
		return rules.length ? new RoutingDispatcher(new Agent(), proxied, rules) : proxied;
	}
	return new Agent();
}
...
export function parseScutilProxy(text) {
	...
	const httpPort = str('HTTPPort');
	const httpsPort = str('HTTPSPort');
	const socksPort = str('SOCKSPort');
 ...
 * 1. `HTTP_PROXY`/`HTTPS_PROXY` (+ `NO_PROXY`) env vars — the cross-platform
 *    baseline every proxy tool on macOS/Linux exports.
...
export function detectSystemProxy(platform = process.platform, env = process.env) {
	...
	if (httpProxy || httpsProxy) return { httpProxy, httpsProxy, noProxy, pac: false };
