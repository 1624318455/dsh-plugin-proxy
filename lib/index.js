...
const PROXY_AGENT_FACTORIES = {
	http: (url) => new EnvHttpProxyAgent({ httpProxy: url, httpsProxy: url }),
	https: (url) => new EnvHttpProxyAgent({ httpProxy: url, httpsProxy: url }),
...
/** Split a request origin (string or URL) into hostname/port. @returns {OriginParts | null} */
...
function exportEnv(config, snapshot) {
	...
	for (const key of NO_PROXY_ENV_KEYS) {
		if (next[key] !== null) continue; // operator value, never clobber
		if (noProxy) process.env[key] = noProxy;
		else delete process.env[key]; // clear a previously exported bypass list
	}
	return next;
}
