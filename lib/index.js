/**
 * @1624318455/dsh-plugin-proxy — runtime-switchable outbound proxy for the DeepSeek Harness.
 *
...
/** Win32 env blocks are case-insensitive: upper/lower variants alias one slot. */
const IS_WIN32 = typeof process !== 'undefined' && process.platform === 'win32';
/** Canonical key per case-variant group on win32 (identity elsewhere). */
const CANONICAL_ENV_KEY = {
	HTTP_PROXY: 'HTTP_PROXY', http_proxy: 'HTTP_PROXY',
	HTTPS_PROXY: 'HTTPS_PROXY', https_proxy: 'HTTPS_PROXY',
	ALL_PROXY: 'ALL_PROXY', all_proxy: 'ALL_PROXY',
	NO_PROXY: 'NO_PROXY', no_proxy: 'NO_PROXY',
};
/** Bookkeeping key: canonical on win32 so aliasing slots snapshot/restore once. */
function envSlot(key) {
	return IS_WIN32 ? (CANONICAL_ENV_KEY[key] ?? key) : key;
}
...
function snapshotEnv() {
	...
	for (const key of PROXY_ENV_KEYS) {
		...
		if (slot in snapshot) continue;
		snapshot[slot] = slot in process.env ? process.env[slot] : null;
	}
	return snapshot;
}
...
function restoreEnv(snapshot) {
	for (const [slot, prior] of Object.entries(snapshot)) {
		if (prior === null) delete process.env[slot];
		else process.env[slot] = prior;
	}
}
...
function createEnvManager() {
	...
	function sync(effective) {
		...
		for (const key of PROXY_ONLY_ENV_KEYS) {
			const slot = envSlot(key);
			adoptOperatorValue(next, slot);
			if (next[slot] === null) {
				process.env[slot] = effective.proxy;
				exported[slot] = effective.proxy;
			}
		}
		for (const key of NO_PROXY_ENV_KEYS) {
			...
			if (noProxy) {
				process.env[slot] = noProxy;
				exported[slot] = noProxy;
			} else {
				delete process.env[slot]; // clear a previously exported bypass list
				delete exported[slot];
			}
		}
		snapshot = next;
	}
